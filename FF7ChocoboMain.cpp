...
void patch_setChocoboInfo() {
	//~~ chocobo's name ~~
	memcpy(D_00DC0ADC, "ABCDEF", 6);
	//~~ cloud/cid/tifa ~~
	D_00DC0AE2 = 0;
	//~~ long/short race ~~
	D_00DC0AE3 = 1;
	...
	D_00DC0AE5 = 0;
	//~~ top speed ~~
	unsigned top_speed = 4500;
	D_00DC0AE6 = top_speed & 0xff; D_00DC0AE7 = top_speed >> 8;
	//~~ speed(2) ~~
	unsigned speed2 = 3500;
	D_00DC0AE8 = speed2 & 0xff; D_00DC0AE9 = speed2 >> 8;
	//~~ ~~
	D_00DC0AEA = 70;
	//~~ obedience? ~~
	D_00DC0AEB = 100;
	...
	D_00DC0AF5 = 0;
	//~~ stamina ~~
	unsigned stamina = 6000;
	D_00DC0AF6 = stamina & 0xff; D_00DC0AF7 = stamina >> 8;
	//~~ ~~
	D_00DC0AF8 = 0;
}
////////////////////////////////////////
...
////////////////////////////////////////
//====---- chocobo ----====
extern void C_0076DBD1(struct t_aa0 *);//chocobo[UPDATE][callback]
extern void C_0076DCAB(int, int, int, struct t_aa0 *);//chocobo[ONMOUSE][callback]
extern void C_0076DCB5(int, int, int, struct t_aa0 *);//chocobo[ONKEY][callback]
extern void C_0076D597(struct t_aa0 *);//chocobo[BEGIN][callback]
extern void C_0076DA88(struct t_aa0 *);//chocobo[END][callback]
////////////////////////////////////////
extern void C_0076B850(const char *);//chocobo:set path
...
int C_004082BF() {
	...
	//...
	//7 - chocobo.lgp
	strcpy(lolo.local_64, C_004076FE());
	strcat(lolo.local_64, "chocobo.lgp");
...
int C_004089C5(struct t_aa0 *bp08) {
	struct {
		...
		int dwGrMode;//local_131
		int local_130;
		char local_129[256];
		int dwSWRenderer;//local_65
		char local_64[256];
	}lolo;
	...
	if(C_0067806E(bp08)) {//graphic driver:START?
		...
		//...
#if 1	//##### for condor/chocobo/field #####
		lolo.dwGrMode = C_00404D80();
		if(bp08->f_a60 == 1)
			lolo.dwSWRenderer = 1;
		C_0067453A(0);//rsd:set "use PSX resources" flag
//		C_0060D837(lolo.dwSWRenderer, lolo.dwGrMode);//ad_app:set screen info
		C_0076D320(lolo.dwSWRenderer, lolo.dwGrMode);//ch_app:set screen info
//		C_005F4630(lolo.dwSWRenderer, lolo.dwGrMode);//condor:set screen info
		//-- set some pathes --
		C_0076B850(C_004076FE());//chocobo:set path
//		C_005F2F18(C_0040776F());//condor:set path
//		strcpy(lolo.local_64, D_009A06B4);
//		strcat(lolo.local_64, "field/");
//		C_0060DF96(bp08, lolo.local_64, lolo.local_64);//ad_app:init
		//-- --
		//...
//		C_00401018();//init:load kernel files
//		C_006CC446();
		//...
#endif
		//-- some patches --
		patch_loadSaveFile(0, 0);
		patch_setChocoboInfo();
	}
#if 1
...
void C_00409DF1(int uMsg, int wParam, int lParam, struct t_aa0 *bp14) {
	C_0076DCAB(uMsg, wParam, lParam, bp14);//chocobo[ONMOUSE][callback]
}
...
#endif
	C_0076DCB5(uMsg, wParam, lParam, bp14);//chocobo[ONKEY][callback]
}
...
////////////////////////////////////////
//for the linker(needed by FF7MENU.LIB)
////////////////////////////////////////
#if 1
int D_00E045EC;
...
void C_0074BE49(struct t_aa0 *p) {
}
//WORLD MAP[ONMOUSE][callback]
void C_0074BA80(int uMsg, int wParam, int lParam, struct t_aa0 *p) {
}
//WORLD MAP[ONKEY][callback]
void C_0074BA85(int uMsg, int wParam, int lParam, struct t_aa0 *p) {
}
//WORLD MAP[BEGIN][callback]
void C_0074BAF5(struct t_aa0 *p) {
}
//WORLD MAP[END][callback]
void C_0074BD77(struct t_aa0 *p) {
}
//====---- ----====
#else
//if not linking with FF7MENU.LIB, you can choose to
// implement those functions.
//(I put a quick&dirty implementation for fun)

struct t_loadmenu_10f4 D_00DBFD38;
unsigned char *D_00DB958C;//FONT_INFO

//"Exit Box"
void C_006C0E2D(struct t_aa0 *p) {
}
...
void C_006C4946(int a, int b) {}

struct tStringListElement {
	struct tStringListElement *pNext;
	char *pString;
	int dwX, dwY;
...
struct tStringListElement *PStringListHead = 0;

//menu.drawString[HIRES]
void C_006F5B03(int dwX, int dwY, unsigned char const *pStr, unsigned char bColor, float fZ) {
	struct tStringListElement *pList = (struct tStringListElement *)malloc(sizeof(struct tStringListElement));
	if(pList) {
		pList->pNext = 0;
		...
		pList->dwY = dwY;
		pList->bColor = bColor;
		pList->fZ = fZ;//we totally ignore this

		int len;
		for(len = 0; pStr[len] != 0xff; len ++);
		len ++;
		pList->pString = (char *)malloc(len);
		if(pList->pString) {
			char *pDst = pList->pString;
			for(int i = 0; pStr[i] != 0xff; i ++) {
				*pDst = pStr[i] + 0x20;
				pDst ++;
			}//end for
			*pDst = 0;
		...
		pList->pNext = PStringListHead;
		PStringListHead = pList;
	}
}

//menu.drawString[LORES]
void C_006FAB2F(int dwX, int dwY, unsigned char const *pStr, unsigned char bColor, float fZ) {
}

//menu.refresh strings?
void C_006FDC26(struct t_aa0 *p) {
	HDC hDC = GetDC(p->hWnd);

	SetBkMode(hDC, TRANSPARENT);

	struct tStringListElement *pList;
	while(pList = PStringListHead) {
		if(pList->pString) {
			//shadow
			SetTextColor(hDC, RGB(0x1f,0x1f,0x1f));
			TextOut(hDC, pList->dwX - 1, pList->dwY - 1, pList->pString, lstrlen(pList->pString));
			//-- --
			assert(pList->bColor == 7);
			SetTextColor(hDC, RGB(0xff,0xff,0xff));
			TextOut(hDC, pList->dwX, pList->dwY, pList->pString, lstrlen(pList->pString));
			//-- --
			free(pList->pString);
		}
		PStringListHead = pList->pNext;
		free(pList);
	}//end for
